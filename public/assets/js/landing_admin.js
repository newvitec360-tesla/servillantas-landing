...
document.getElementById('saveAllBtn').addEventListener('click', async () => {
  ...
  // Siempre guardamos localmente por si acaso
  localStorage.setItem('landing_draft_local', JSON.stringify(snapshot));
  ...
  try {
    const res = await fetch('/landing-gestor/borrador', {
      method: 'POST',
    ...
    if(data.success) {
      ...
      showToast('🔴 No se pudo guardar el borrador en la base de datos. Los cambios no fueron sincronizados.');
    }
  } catch (e) {
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    const healthRes = await fetch('/landing-gestor/health');
    ...
    if (healthData.success && healthData.database === 'connected') {
      ...
      isLocalMode = false;
      if(statusText) {
         statusText.innerHTML = '🟢 Gestor conectado a la base de datos';
    ...
    const statusText = document.getElementById('connection-status-text');
    if(statusText) {
       statusText.innerHTML = '🔴 Sin conexión con la API. Los cambios solo se guardarán temporalmente en este navegador.';
...
async function loadDraftFromAPI() {
  try {
    ...
    console.error('[LANDING_LOAD_ERROR] No se pudo cargar el borrador', e);
    loadDraftFromLocal();
  }
}
...
function loadDraftFromLocal() {
    try {
        const local = localStorage.getItem('landing_draft_local');
        if (local) {
            populateFields(JSON.parse(local));
            showToast('Borrador cargado desde caché local');
        }
    } catch(e) {
        console.error('[LANDING_LOCAL_ERROR] Fallo carga local', e);
    }
}
...
if(pubBtn) {
  pubBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/landing-gestor/publicar', { method: 'POST' });
      const data = await res.json();
      if(data.success) {
        showToast('🟢 Cambios publicados correctamente en la landing.');
      } else {
        console.error("[LANDING_PUBLISH_ERROR]", data.message);
        showToast('🔴 No se pudo publicar. Verifique la conexión con la API o la base de datos.');
      }
    } catch (e) {
